...
def _path_to_token_seq(path_steps: List[List[int]]) -> List[str]:
    # Keep if you still want to train on path supervision; otherwise tail-only will skip this.
    toks: List[str] = []
    for (h, r, t) in path_steps:
        toks.append(f"R{int(r)}")
        toks.append(str(int(t)))
...
def qa_batch_to_squire_samples(
    ...
    device: torch.device,
    *,
    no_relations: bool = True,          # <-- default: relation-free mode
    tail_only: bool = True,             # <-- default: predict only the final tail
    bos_when_missing_relation: bool = False,  # kept for API compatibility; unused in no-relations
) -> Dict[str, Any]:
    """
    Adapt a QA batch to SQUIRE tensors without consuming any relation column.
    - Does NOT read batch["query_rel"].
    - Builds `source` with a neutral placeholder in the relation slot to keep shape stable.
    - Uses tail-only objective by default.
    """
    bsz = batch["question_ids"].size(0)
    bos = dictionary.bos()
    eos = dictionary.eos()
    ...
    # --- Build `source` = [<s>, head, NO_REL] (NO_REL is `unk` to neutralize any filtering) ---
    head_ids = [ _to_dict_id(dictionary, str(int(e))) for e in batch["query_ent"].tolist() ]
    no_rel_ids = torch.full((bsz,), unk, dtype=torch.long)  # neutral placeholder
    source = torch.stack([
        torch.full((bsz,), bos, dtype=torch.long),
        torch.tensor(head_ids, dtype=torch.long),
        no_rel_ids,
    ...
    # --- Build target/prev_outputs/mask ---
    targets: List[torch.Tensor] = []
    lengths: List[int] = []
    ...
    for i in range(bsz):
        if tail_only:
            ...
            # (Optional) If you still want path supervision, derive from `paths`.
            steps: List[List[int]] = batch["paths"][i]
            if len(steps) == 0:
                # With no relations, don't synthesize a fake R; fall back to tail-only for this row.
                t = int(batch["answer_ent"][i].item())
                seq_tokens = [str(t)]
            else:
                seq_tokens = _path_to_token_seq(steps)
    ...
    return {
        ...
        "target":        target.to(device),
        "mask":          mask.to(device),
        # pass-through question tensors
        "question_ids":            batch["question_ids"].to(device),
        "question_attention_mask": batch["question_attention_mask"].to(device),
        # bookkeeping (relation intentionally omitted)
        "query_ent":  batch["query_ent"].to(device),
        "answer_ent": batch["answer_ent"].to(device),
        "hops":       batch["hops"].to(device),
    }

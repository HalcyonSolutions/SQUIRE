...
class Question2PathModel(nn.Module):
    ...
    #! NEW
    def generate_with_paths(self, question_ids, max_len=10):
        """
        Greedy generation:
        Returns:
        predicted_tail_id   : int
        path_ids            : list[int] (h, r, t, h, r, t, ...)
        """

        device = question_ids.device
        attention_mask = (question_ids != self.tokenizer.pad_token_id).long()

        # <bos> is index 1 in SQUIRE dictionaries
        ...
        for step in range(max_len):
            ...
            path_tokens.append(next_id)

            prev = torch.cat([prev, torch.tensor([[next_id]], device=device)], dim=1)

        # path_tokens is raw: [h, r, t, h, r, t,...]
        # prediction = last entity
        if len(path_tokens) == 0:
            pred = -1
        else:
            pred = path_tokens[-1]

        return pred, path_tokens

...
import math
from model_text_input import Text2PathModel
from dictionary import Dictionary
from dataset_question_input import load_dictionary_or_init
...
def get_args():
    ...
    parser.add_argument("--load-ckpt", default=False, action="store_true") # load from checkpoint

    # * NEW
    parser.add_argument("--text-input", default=False, action="store_true",
                        help="Use text encoder for the source side")
    parser.add_argument("--text-encoder-name", default="distilbert-base-uncased",
                        help="HF model name for the source text encoder")
    parser.add_argument("--max-text-len", default=64, type=int,
                        help="Max tokens for the text prompt")
    parser.add_argument("--unfreeze-text-encoder", default=False, action="store_true",
                        help="Fine-tune the text encoder weights")
...
def evaluate(model, dataloader, device, args, true_triples=None, valid_triples=None):
    ...
    mrr, hit, hit1, hit3, hit5, hit10, hit20, count = (0, 0, 0, 0, 0, 0, 0, 0)
    ...
    with tqdm(dataloader, desc="testing") as pbar:
        for samples in pbar:
            pbar.set_description("MRR: %f, Hit@1: %f, Hit@3: %f, Hit@5: %f, Hit@10: %f, Hit@20: %f" % (mrr/max(1, count), hit1/max(1, count), hit3/max(1, count), hit5/max(1, count), hit10/max(1, count), hit20/max(1, count)))
    ...
    if args.output_path:
        with open("test_output_squire.txt", "w") as f:
            f.writelines(lines)
    logging.info("[MRR: %f] [Hit@1: %f] [Hit@3: %f] [Hit@5: %f] [Hit@10: %f] [Hit@20: %f]" % (mrr/count, hit1/count, hit3/count, hit5/count, hit10/count, hit20/count))
    return hit/count, hit1/count, hit3/count, hit5/count, hit10/count, hit20/count

def train(args):
    dictionary = load_dictionary_or_init(args.vocab_path, args.data_path)
    ...
    # * NEW
    if args.text_input:
        model = Text2PathModel(
            args,
            train_set.dictionary,
            text_model_name=args.text_encoder_name,
    ...
    hf_emb = model.text_model.get_input_embeddings()
    base_params = [p for n,p in model.named_parameters()
                if p.requires_grad and not n.startswith("text_model.embeddings")]
    optimizer = optim.Adam([
        {"params": base_params, "lr": args.lr},
        {"params": hf_emb.parameters(), "lr": min(args.lr, 5e-5)},  # small LR for HF emb
    ...
    steps = 0
    if args.load_ckpt:
        if os.path.exists(os.path.join(ckpt_path, args.ckpt)):
            checkpoint = torch.load(os.path.join(ckpt_path, args.ckpt))
    ...
    for epoch in range(args.num_epoch):
        ...
        with tqdm(train_loader, desc="training") as pbar:
            ...
            for samples in pbar:
                ...
                #       f"source: {samples['source'].shape}, " +
                #       f"target: {samples['target'].shape}, " +
                #       f"mask: {samples['mask'].shape}\n\n")
                # # print("Continue? (y/n): ")
                # choice = input()
                # if choice.lower() != 'y':
                #     exit(0)
        ...
        if (epoch % args.save_interval == 0 and epoch != 0) or (epoch == args.num_epoch - 1):
            # save model weights
            torch.save(model.state_dict(), ckpt_path + "/ckpt_{}.pt".format(epoch + 1))
            # save tokenizer to keep special-token IDs stable
            if args.text_input:
                tok_dir = os.path.join(save_path, "tokenizer")
                os.makedirs(tok_dir, exist_ok=True)
                model.tokenizer.save_pretrained(tok_dir)
            with torch.no_grad():
                 evaluate(model, test_loader, device, args, train_valid, eval_valid)
...
def checkpoint(args):
    ...
    if args.text_input:
        ...
        # reload the exact tokenizer used during training
        tok_dir = os.path.join(save_path, "tokenizer")
        if os.path.isdir(tok_dir):
            model.tokenizer = transformers.AutoTokenizer.from_pretrained(tok_dir, use_fast=True)
            model.text_model.resize_token_embeddings(len(model.tokenizer))
    else:

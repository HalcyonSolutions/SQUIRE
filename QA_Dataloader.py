...
class SupervisedQADataset(Dataset):
    def __init__(
        ...
        assert "Source-Entity" in self.df.columns  and "Answer-Entity" in self.df.columns, \
            "Missing Source-Entity/Answer-Entity columns"

        Qr_missing = False
        if "Query-Relation" not in self.df.columns:
            Qr_missing = True
        ...
        if FULL_dataset:
            all_splits = sorted(self.df[split_column].unique().tolist())
            norm_split = self.df[split_column].astype(str).str.strip().str.lower()
            self.df = self.df[norm_split.isin(all_splits)].reset_index(drop=True)
            if len(self.df) == 0:
        ...
        if not Qr_missing:
            ...
            # If missing, default to a dummy relation (e.g., 0)
            # TODO: Map it to 9999
            self.df["Query-Relation"] = 0
...
def make_train_test_dataloaders(
    ...
    # When FULL_dataset is True, splits are ignored
    train_ds = SupervisedQADataset(
        csv_path, entity2id, relation2id, question_tokenizer_name, answer_tokenizer_name,
        split="train", text_only=text_only, FULL_dataset=FULL_dataset
    )
    test_ds = SupervisedQADataset(
        csv_path, entity2id, relation2id, question_tokenizer_name, answer_tokenizer_name,
        split="test", text_only=text_only, FULL_dataset=FULL_dataset
    )

...
# Saved commands
```
CUDA_VISIBLE_DEVICES=0 python train_text_input.py --dataset FB15K237 --trainset 6_rev_rule --max-len 3 --encoder   --embedding-dim 256 --hidden-size 512 --num-layers 6   --batch-size 512 --lr 5e-4 --dropout 0.1   --beam-size 256 --num-epoch 30 --save-interval 5   --save-dir model_text_tokens_proj   --text-input --text-encoder-name distilbert-base-uncased --label-smooth 0.25
```
```
CUDA_VISIBLE_DEVICES=0 python train_text_input.py --dataset kinshiphinton --trainset 6_rev_rule --max-len 3 --encoder   --embedding-dim 256 --hidden-size 512 --num-layers 6   --batch-size 32 --lr 5e-4 --dropout 0.1   --beam-size 32 --num-epoch 30 --save-interval 5   --save-dir model_text_tokens_proj_kinship   --text-input --text-encoder-name distilbert-base-uncased --label-smooth 0.25
```

```
CUDA_LAUNCH_BLOCKING=1 python train_question_input.py   --dataset kinshiphinton   --csv-path data/kinshiphinton/kinship_hinton_qa_1hop.csv --entity2id data/kinshiphinton/entity2id.txt   --relation2id data/kinshiphinton/relation2id.txt   --question-tokenizer facebook/bart-base   --batch-size 32 --num-epoch 20 --beam-size 58 --max-len 1   --save-dir model_q_kinship   --unfreeze-text-encoder --label-smooth 0.1   --l-punish --self-consistency --tail-only --smart-filter
```

```
CUDA_LAUNCH_BLOCKING=1 python train_question_input.py   --dataset mquake   --csv-path data/mquake/mquake_qa_2hop.csv --entity2id data/kinshiphinton/entity2id.txt   --relation2id data/kinsh
iphinton/relation2id.txt   --question-tokenizer facebook/bart-base   --batch-size 128 --num-epoch 30 --beam-size 256 --max-len 2   --save-dir model_q_kinship   --unfreeze-text-encoder --label-smooth 0.1   --l-punish --self-consis
tency --tail-only --smart-filter
```
